...
public class Script : ConsoleAppBase, IScript
{
    ...
    public async Task Import([Option("u", "url of yt video")] string videoUrl,
        [Option("o", "output path where yt video should be saved")]
        string? folderPath = default)
    {
        ...
        try
        {
            await _downloader.SaveSingleVideoMP3(videoUrl, false, folderPath, Context.CancellationToken);
    ...
    public async Task ImportPlaylist([Option("u", "url of yt playlist")] string playlistUrl,
        [Option("o", "output path where yt playlist should be saved")]
        string? folderPath = default,
    ...
    )
    {
        ...
        try
        {
            if (!await _downloader.SavePlaylistMP3(playlistUrl, folderPath, false, Context.CancellationToken))
            {
